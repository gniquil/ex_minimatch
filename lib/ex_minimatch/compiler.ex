defmodule ExMinimatch.Compiler do
  ...
  def parse_glob_to_re(glob_pattern, options, is_sub \\ false) do
    ...
    |> parse
    |> minimatch_tap(fn state -> debug({"parse", state}, state[:options]) end)
    |> handle_open_class
    |> minimatch_tap(fn state -> debug({"handle_open_class", state}, state[:options]) end)
    |> handle_weird_end
    |> minimatch_tap(fn state -> debug({"handle_weird_end", state}, state[:options]) end)
    |> handle_trailing_things
    |> minimatch_tap(fn state -> debug({"handle_trailing_things", state}, state[:options]) end)
    |> handle_dot_start
    |> minimatch_tap(fn state -> debug({"handle_dot_start", state}, state[:options]) end)
    |> finish_parse
  end
  ...
  def continue(state) do
    state
    |> minimatch_tap(fn %{options: options} = state -> info({"continue", state}, options) end)
    |> move_to_next
    |> parse
  end
end
